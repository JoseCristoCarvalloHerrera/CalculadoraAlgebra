...
=====================================================================
 ...
 Módulo de vectores en R^n (Programa 3):
   ...
   - Determina si un vector b es combinación lineal de un conjunto de
     vectores {v1, v2, ..., vk}, y devuelve los pesos cuando lo es.
 ...
 Módulo de Operaciones Matriciales Básicas (Requerimiento Funcional 2):
   - Suma y resta de matrices m x n.
   - Multiplicación de una matriz por un escalar.
   - Multiplicación de matrices A (m x n) * B (n x p) mediante bucles anidados.
...
# =====================================================================
# BLOQUE 1A: OPERACIONES MATRICIALES BÁSICAS (REQUERIMIENTO FUNCIONAL 2)
# Suma, resta, producto por escalar y multiplicación A*B con 3 bucles anidados.
# =====================================================================
def sumar_matrices(A, B):
    """Suma dos matrices A y B comprobando dimensiones iguales m x n."""
    if len(A) != len(B) or len(A[0]) != len(B[0]):
        raise ValueError(f"Dimensión incompatible: A es {len(A)}x{len(A[0])} y B es {len(B)}x{len(B[0])}. Deben ser de igual dimensión.")
    return [[A[i][j] + B[i][j] for j in range(len(A[0]))] for i in range(len(A))]


def restar_matrices(A, B):
    """Resta dos matrices A y B comprobando dimensiones iguales m x n."""
    if len(A) != len(B) or len(A[0]) != len(B[0]):
        raise ValueError(f"Dimensión incompatible: A es {len(A)}x{len(A[0])} y B es {len(B)}x{len(B[0])}. Deben ser de igual dimensión.")
    return [[A[i][j] - B[i][j] for j in range(len(A[0]))] for i in range(len(A))]
...
def escalar_por_matriz(c, A):
    """Multiplica una matriz A por un escalar c."""
...
def multiplicar_matrices(A, B):
    ...
    Algoritmo mediante tres bucles anidados i, j, k.
    Validación estricta: columnas de A == filas de B.
    """
    m = len(A)
    n = len(A[0])
    filas_B = len(B)
    ...
    if n != filas_B:
        raise ValueError(f"Dimensión incompatible: A tiene {n} columnas y B tiene {filas_B} filas.")
...
def escalonar(matriz, col_barra=None):
    ...
    # Fase 2 (Gauss-Jordan) -> Ceros por encima de los pivotes (Búsqueda dinámica)
    pasos.append("--- FASE 2: GAUSS-JORDAN (Ceros arriba de los pivotes) ---")
    pasos.append("")
    hubo_operaciones_arriba = False

    for f in range(filas - 1, -1, -1):
        c_piv = entrada_principal(matriz[f])
        if c_piv is not None and c_piv < columnas - 1:
            for f_arriba in range(f - 1, -1, -1):
                if matriz[f_arriba][c_piv] != 0:
                    factor = matriz[f_arriba][c_piv]
                    pasos.append(reemplazar_fila(matriz, f_arriba, factor, f))
                    pasos.extend(formato_matriz(matriz, col_barra))
                    pasos.append("")
    ...
    pivotes_actualizados = []
    for f in range(filas):
        c = entrada_principal(matriz[f])
        if c is not None and c < columnas - 1:
            pivotes_actualizados.append((f, c))

    return pasos, pivotes_actualizados
...
class VectoresApp:
    ...
    def __init__(self, raiz, callback_volver):
        ...
        self.celdas_b = {}      # fila -> StringVar del vector b
    ...
    def _construir_ui(self):
        tk.Button(self.marco_principal, text="← Volver al Menú", font=self.fuente_body,
                  ...
                  ).grid(row=0, column=0, sticky="w", padx=34, pady=(10, 0))
        ...
        tk.Label(self.marco_principal,
                 text="Operaciones vectoriales, combinación lineal e independencia lineal",
                 font=self.fuente_sub, bg=FONDO, fg=TEXTO_SUAVE
                 ).grid(row=2, column=0, columnspan=2, sticky="w", padx=34, pady=(0, 14))
        ...
        tarjeta = self._crear_tarjeta(panel_izq)
        tarjeta.pack(side="top", fill="both", expand=True)
        self._llenar_cabecera(tarjeta)

        self.frame_vectores = tk.Frame(tarjeta, bg=TARJETA)
        self.frame_vectores.pack(fill="both", expand=True, padx=18, pady=(0, 10))

        tk.Label(tarjeta, text=AYUDA_NUMERO, font=("Montserrat", 9),
                 bg=TARJETA, fg=TEXTO_SUAVE, wraplength=380, justify="left"
        ...
        tk.Label(tarjeta_res, text="Resultado", font=self.fuente_encab,
                 bg=TARJETA, fg=TEXTO_SUAVE).pack(anchor="w", padx=18, pady=(14, 6))

        self.frame_resultado = tk.Frame(tarjeta_res, bg=TARJETA)
        self.frame_resultado.pack(fill="both", expand=True, padx=18, pady=(0, 16))
    ...
    def _llenar_cabecera(self, tarjeta):
        ...
        tk.Label(cont, text="Vectores — cada columna es un vector",
        ...
        botones.pack(fill="x", padx=18, pady=(4, 10))
        acciones = [
            ("¿b es combinación lineal?", self._al_combinacion),
            ("¿Son independientes?", self._al_independencia),
            ("v₁ + v₂", self._al_sumar),
            ("v₁ − v₂", self._al_restar),
        ]
        ...
        fila_esc.pack(fill="x", padx=18, pady=(0, 10))
        tk.Label(fila_esc, text="Escalar c =", font=self.fuente_body,
        ...
        tk.Button(fila_esc, text="c · v₁", font=("Montserrat", 10, "bold"),
                  bg=BOTON_SEC, fg=TEXTO, relief="flat", cursor="hand2",
                  padx=10, pady=5, activebackground=BOTON_SEC_HOVER,
                  command=self._al_escalar).pack(side="left", padx=4)
    ...
    def _construir_grid_vectores(self):
        ...
        tk.Label(self.frame_vectores, text="b", font=self.fuente_encab,
        ...
        for i in range(n):
            ...
            var_b = tk.StringVar(value=previos_b.get(i, ""))
            self.celdas_b[i] = var_b
            tk.Entry(self.frame_vectores, textvariable=var_b, font=self.fuente_mono,
                     width=6, justify="center", relief="solid", bd=1,
    ...
    def _leer_b(self):
        n = self._leer_dimension(self.var_n, 3)
        return [a_numero(self.celdas_b[i].get()) for i in range(n)]
    ...
    def _al_sumar(self):
        try:
            ...
            self._mostrar("v₁ + v₂", formato_vector(r),
                          "Se suman las entradas correspondientes (Sesión 5).")
    ...
    def _al_restar(self):
        try:
            ...
            self._mostrar("v₁ − v₂", formato_vector(r),
                          "La resta se calcula como v₁ + (−1)v₂ (propiedad iv de Rⁿ).")
        except ValueError as e:
            self._mostrar_error(str(e))
    ...
    def _al_escalar(self):
        try:
            ...
            self._mostrar(formato(c) + " · v₁", formato_vector(r),
                          "Se multiplica por el escalar cada entrada del vector.")
        except ValueError as e:
            self._mostrar_error(str(e))
    ...
    def _al_combinacion(self):
        try:
            vs = self._leer_vectores()
            b = self._leer_b()
            es_cl, pesos, resultado = es_combinacion_lineal(b, vs)
            if not es_cl:
                self._mostrar("NO es combinación lineal",
                              "El sistema [v₁ … v_k | b] resultó inconsistente.",
                              "No existen pesos que generen b a partir de esos vectores.",
                              color=ERROR)
                return
            if resultado["clasificacion"] == "Consistente Determinado":
                texto = "  ".join(
                    con_subindice("c" + str(i + 1)) + " = " + formato(p)
                    for i, p in enumerate(pesos))
                detalle = "b = " + "  +  ".join(
                    formato(p) + "·" + con_subindice("v" + str(i + 1))
                    for i, p in enumerate(pesos))
                self._mostrar("SÍ es combinación lineal", texto, detalle, color=EXITO)
            else:
                self._mostrar("SÍ es combinación lineal",
                              "Hay infinitas formas de escribirla.",
                              "El sistema es consistente indeterminado: hay variables libres.",
                              color=ADVERTENCIA)
        except ValueError as e:
            self._mostrar_error(str(e))
    ...
    def _al_independencia(self):
        try:
            vs = self._leer_vectores()
            independientes, resultado = son_linealmente_independientes(vs)
            if independientes:
                self._mostrar("LINEALMENTE INDEPENDIENTES",
                              "c₁v₁ + … + c_kv_k = 0 solo con todos los c en cero.",
                              "El sistema homogéneo no tiene variables libres, "
                              "así que solo admite la solución trivial.", color=EXITO)
            else:
                libres = ", ".join(con_subindice("c" + str(c + 1))
                                   for c in resultado["variables_libres"])
                self._mostrar("LINEALMENTE DEPENDIENTES",
                              "Existe una solución no trivial.",
                              "Hay variables libres (" + libres + "), así que por el teorema "
                              "de la Sesión 7 la ecuación homogénea tiene solución no trivial.",
                              color=ADVERTENCIA)
        except ValueError as e:
            self._mostrar_error(str(e))
    ...
    def _limpiar_resultado(self):
        for hijo in self.frame_resultado.winfo_children():
            hijo.destroy()
        tk.Label(self.frame_resultado,
                 text="Llená los vectores y elegí una operación.",
                 font=self.fuente_body, bg=TARJETA, fg=TEXTO_SUAVE,
                 wraplength=420, justify="left").pack(anchor="w")

    def _mostrar(self, titulo, texto, detalle="", color=ACENTO):
        for hijo in self.frame_resultado.winfo_children():
            hijo.destroy()
        ...
        if detalle:
            tk.Label(self.frame_resultado, text=detalle, font=self.fuente_body,
                     bg=TARJETA, fg=TEXTO_SUAVE, wraplength=440, justify="left"
                     ).pack(anchor="w")

    def _mostrar_error(self, mensaje):
        self._mostrar("Revisá los datos", mensaje, "", color=ERROR)
...
# =====================================================================
# BLOQUE V3: PANTALLA DEL MÓDULO DE OPERACIONES MATRICIALES (NUEVA PANTALLA)
# Diseñada manteniendo exactamente la misma maquetación visual de tarjetas,
# colores y fuentes que el resto de la aplicación.
# =====================================================================
class OperacionesMatricialesApp:
    """Pantalla del módulo de Operaciones Matriciales Básicas (A+B, A-B, kA, A*B)."""

    def __init__(self, raiz, callback_volver):
        self.raiz = raiz
        self.callback_volver = callback_volver

        self.var_mA = tk.StringVar(value="2")
        self.var_nA = tk.StringVar(value="2")
        self.var_mB = tk.StringVar(value="2")
        self.var_nB = tk.StringVar(value="2")
        self.var_escalar = tk.StringVar(value="3")
    ...
    def _construir_ui(self):
        ...
        tk.Label(self.marco_principal, text="Módulo de Operaciones Matriciales Básicas",
                 font=self.fuente_titulo, bg=FONDO, fg=TEXTO
                 ).grid(row=1, column=0, columnspan=2, sticky="w", padx=34, pady=(5, 4))
        tk.Label(self.marco_principal,
                 text="Suma, resta, multiplicación por escalar y multiplicación de matrices A · B",
                 font=self.fuente_sub, bg=FONDO, fg=TEXTO_SUAVE
                 ).grid(row=2, column=0, columnspan=2, sticky="w", padx=34, pady=(0, 14))
        ...
        # Matriz A dimensiones
        tk.Label(cont, text="Matriz A (m x n):", font=self.fuente_encab, bg=TARJETA, fg=TEXTO).grid(row=0, column=0, columnspan=2, sticky="w", pady=(0, 2))
        tk.Spinbox(cont, from_=1, to=8, textvariable=self.var_mA, width=3, font=self.fuente_body, justify="center", bg=FONDO, fg=TEXTO, relief="solid", bd=1, command=self._construir_grids).grid(row=1, column=0, sticky="w", padx=(0, 4))
        tk.Label(cont, text="x", font=self.fuente_body, bg=TARJETA, fg=TEXTO_SUAVE).grid(row=1, column=1, sticky="w", padx=(0, 4))
        tk.Spinbox(cont, from_=1, to=8, textvariable=self.var_nA, width=3, font=self.fuente_body, justify="center", bg=FONDO, fg=TEXTO, relief="solid", bd=1, command=self._construir_grids).grid(row=1, column=2, sticky="w", padx=(0, 20))

        # Matriz B dimensiones
        tk.Label(cont, text="Matriz B (n x p):", font=self.fuente_encab, bg=TARJETA, fg=TEXTO).grid(row=0, column=3, columnspan=2, sticky="w", pady=(0, 2))
        tk.Spinbox(cont, from_=1, to=8, textvariable=self.var_mB, width=3, font=self.fuente_body, justify="center", bg=FONDO, fg=TEXTO, relief="solid", bd=1, command=self._construir_grids).grid(row=1, column=3, sticky="w", padx=(0, 4))
        tk.Label(cont, text="x", font=self.fuente_body, bg=TARJETA, fg=TEXTO_SUAVE).grid(row=1, column=4, sticky="w", padx=(0, 4))
        tk.Spinbox(cont, from_=1, to=8, textvariable=self.var_nB, width=3, font=self.fuente_body, justify="center", bg=FONDO, fg=TEXTO, relief="solid", bd=1, command=self._construir_grids).grid(row=1, column=5, sticky="w")

        # Grids de edición de A y B
        self.frame_grids = tk.Frame(tarjeta, bg=TARJETA)
        self.frame_grids.pack(fill="both", expand=True, padx=18, pady=10)

        # Botones de Operación
        botones = tk.Frame(tarjeta, bg=TARJETA)
        botones.pack(fill="x", padx=18, pady=(4, 10))
        
        tk.Button(botones, text="A + B", font=("Montserrat", 10, "bold"), bg=BOTON_SEC, fg=TEXTO, relief="flat", cursor="hand2", padx=8, pady=6, activebackground=BOTON_SEC_HOVER, command=self._al_sumar).grid(row=0, column=0, sticky="ew", padx=3, pady=3)
        tk.Button(botones, text="A − B", font=("Montserrat", 10, "bold"), bg=BOTON_SEC, fg=TEXTO, relief="flat", cursor="hand2", padx=8, pady=6, activebackground=BOTON_SEC_HOVER, command=self._al_restar).grid(row=0, column=1, sticky="ew", padx=3, pady=3)
        tk.Button(botones, text="A · B (Multiplicar)", font=("Montserrat", 10, "bold"), bg=ACENTO, fg=FONDO, relief="flat", cursor="hand2", padx=8, pady=6, activebackground=ACENTO_HOVER, command=self._al_multiplicar).grid(row=1, column=0, columnspan=2, sticky="ew", padx=3, pady=3)
        
        botones.columnconfigure(0, weight=1)
        botones.columnconfigure(1, weight=1)

        fila_esc = tk.Frame(tarjeta, bg=TARJETA)
        fila_esc.pack(fill="x", padx=18, pady=(0, 12))
        tk.Label(fila_esc, text="Escalar k =", font=self.fuente_body, bg=TARJETA, fg=TEXTO_SUAVE).pack(side="left")
        tk.Entry(fila_esc, textvariable=self.var_escalar, font=self.fuente_mono, width=6, justify="center", relief="solid", bd=1).pack(side="left", padx=6)
        tk.Button(fila_esc, text="k · A", font=("Montserrat", 10, "bold"), bg=BOTON_SEC, fg=TEXTO, relief="flat", cursor="hand2", padx=10, pady=4, activebackground=BOTON_SEC_HOVER, command=self._al_escalar).pack(side="left", padx=4)
        ...
        tk.Label(tarjeta_res, text="Resultado", font=self.fuente_encab, bg=TARJETA, fg=TEXTO_SUAVE).pack(anchor="w", padx=18, pady=(14, 6))
    ...
    def _construir_grids(self):
        mA = max(1, min(8, int(self.var_mA.get() or 2)))
        nA = max(1, min(8, int(self.var_nA.get() or 2)))
        mB = max(1, min(8, int(self.var_mB.get() or 2)))
        nB = max(1, min(8, int(self.var_nB.get() or 2)))

        for hijo in self.frame_grids.winfo_children(): hijo.destroy()
        self.celdas_A = {}
        self.celdas_B = {}

        # Subframe A
        fA = tk.Frame(self.frame_grids, bg=TARJETA)
        fA.pack(side="left", expand=True, fill="both")
        tk.Label(fA, text="Matriz A", font=self.fuente_encab, bg=TARJETA, fg=TEXTO).pack(anchor="w", pady=(0, 4))
        grid_A = tk.Frame(fA, bg=TARJETA)
        grid_A.pack()

        for i in range(mA):
            for j in range(nA):
                var = tk.StringVar(value="")
                self.celdas_A[(i, j)] = var
                tk.Entry(grid_A, textvariable=var, font=self.fuente_mono, width=5, justify="center", relief="solid", bd=1).grid(row=i, column=j, padx=2, pady=2)

        # Subframe B
        fB = tk.Frame(self.frame_grids, bg=TARJETA)
        fB.pack(side="right", expand=True, fill="both", padx=(10, 0))
        tk.Label(fB, text="Matriz B", font=self.fuente_encab, bg=TARJETA, fg=TEXTO).pack(anchor="w", pady=(0, 4))
        grid_B = tk.Frame(fB, bg=TARJETA)
        grid_B.pack()

        for i in range(mB):
            for j in range(nB):
                var = tk.StringVar(value="")
                self.celdas_B[(i, j)] = var
                tk.Entry(grid_B, textvariable=var, font=self.fuente_mono, width=5, justify="center", relief="solid", bd=1).grid(row=i, column=j, padx=2, pady=2)

    def _leer_matriz(self, celdas, m, n):
        matriz = []
        for i in range(m):
            fila = []
            for j in range(n):
    ...
    def _al_sumar(self):
        try:
            mA, nA = int(self.var_mA.get()), int(self.var_nA.get())
            mB, nB = int(self.var_mB.get()), int(self.var_nB.get())
            A = self._leer_matriz(self.celdas_A, mA, nA)
            B = self._leer_matriz(self.celdas_B, mB, nB)
            R = sumar_matrices(A, B)
            self._mostrar_resultado("Suma de Matrices: A + B", R, f"Resultado de dimensión {mA}x{nA}")
        except ValueError as e:
            self._mostrar_error(str(e))
    ...
    def _al_restar(self):
        try:
            ...
            self._mostrar_resultado("Resta de Matrices: A − B", R, f"Resultado de dimensión {mA}x{nA}")
        except ValueError as e:
            self._mostrar_error(str(e))

    def _al_escalar(self):
        try:
            k = a_numero(self.var_escalar.get())
            mA, nA = int(self.var_mA.get()), int(self.var_nA.get())
            A = self._leer_matriz(self.celdas_A, mA, nA)
            R = escalar_por_matriz(k, A)
            self._mostrar_resultado(f"Producto Escalar: {formato(k)} · A", R, f"Multiplicación cada entrada por {formato(k)}")
        except ValueError as e:
            self._mostrar_error(str(e))

    def _al_multiplicar(self):
        try:
            mA, nA = int(self.var_mA.get()), int(self.var_nA.get())
            mB, nB = int(self.var_mB.get()), int(self.var_nB.get())
            A = self._leer_matriz(self.celdas_A, mA, nA)
            ...
            self._mostrar_resultado("Multiplicación Matricial: A · B", R, f"Matriz resultante de dimensión {mA}x{nB} obtenida por bucles anidados i,j,k")
        except ValueError as e:
            self._mostrar_error(str(e))

    def _limpiar_resultado(self):
        for hijo in self.frame_resultado.winfo_children(): hijo.destroy()
        tk.Label(self.frame_resultado, text="Ingrese los coeficientes de las matrices A y B y pulse una operación.", font=self.fuente_body, bg=TARJETA, fg=TEXTO_SUAVE, wraplength=420, justify="left").pack(anchor="w")

    def _mostrar_resultado(self, titulo, matriz, detalle="", color=ACENTO):
        for hijo in self.frame_resultado.winfo_children(): hijo.destroy()
        tk.Label(self.frame_resultado, text=titulo, font=self.fuente_encab, bg=color, fg="#FFFFFF", padx=12, pady=8, anchor="w").pack(fill="x", pady=(0, 10))
        
        lineas = formato_matriz(matriz)
        texto_matriz = "\n".join(lineas)
        tk.Label(self.frame_resultado, text=texto_matriz, font=self.fuente_mono, bg=TARJETA, fg=TEXTO, justify="left").pack(anchor="w", pady=(0, 8))
        if detalle:
            tk.Label(self.frame_resultado, text=detalle, font=self.fuente_body, bg=TARJETA, fg=TEXTO_SUAVE, wraplength=440, justify="left").pack(anchor="w")

    def _mostrar_error(self, mensaje):
        for hijo in self.frame_resultado.winfo_children(): hijo.destroy()
        tk.Label(self.frame_resultado, text="Dimensión Incompatible / Error", font=self.fuente_encab, bg=ERROR, fg="#FFFFFF", padx=12, pady=8, anchor="w").pack(fill="x", pady=(0, 10))
        tk.Label(self.frame_resultado, text=mensaje, font=self.fuente_body, bg=TARJETA, fg=TEXTO, wraplength=440, justify="left").pack(anchor="w")
...
class MenuPrincipal:
    ...
    def __init__(self, raiz):
        ...
        tk.Label(self.frame_menu, text="¡Bienvenido a la mejor Calculadora!", font=("Montserrat", 26, "bold"), bg=FONDO, fg=TEXTO).pack(pady=(110, 10))
        tk.Label(self.frame_menu, text="Proyecto de Álgebra Lineal", font=("Montserrat", 16), bg=FONDO, fg=TEXTO_SUAVE).pack(pady=(0, 30))
        ...
        btn_matrices_basicas = tk.Button(self.frame_menu, text="Operaciones Matriciales Básicas", font=("Montserrat", 13, "bold"),
                                        bg=ACENTO, fg=FONDO, padx=30, pady=13, relief="flat", cursor="hand2",
                                        activebackground=ACENTO_HOVER, activeforeground=FONDO, command=self.abrir_operaciones_matriciales)
        btn_matrices_basicas.pack(pady=8)
        ...
        btn_matrices.pack(pady=8)

        btn_vectores = tk.Button(self.frame_menu, text="Sistemas de Ecuaciones (Vectores)", font=("Montserrat", 13, "bold"),
                                  bg=ACENTO, fg=FONDO, padx=30, pady=13, relief="flat", cursor="hand2",
                                  activebackground=ACENTO_HOVER, activeforeground=FONDO, command=self.abrir_vectores)
        btn_vectores.pack(pady=8)
    ...
    def abrir_operaciones_matriciales(self):
        self.frame_menu.pack_forget()
        OperacionesMatricialesApp(self.raiz, callback_volver=self.mostrar_menu)
...
class CalculadoraApp:
    ...
    def _mostrar_resultado(self, resultado):
        ...
        for i, (campo, valor) in enumerate(resultado["datos_pizarra"]):
...
# =====================================================================
# PUNTO DE ENTRADA
# Solo le decimos a Python que abra la ventana del menú principal
# =====================================================================
def main():
